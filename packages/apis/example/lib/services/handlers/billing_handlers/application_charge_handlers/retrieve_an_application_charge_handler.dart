...
class RetrieveAnApplicationChargeHandler implements ApiRequestHandler {
  ...
  Future<Map<String, dynamic>> handleRequest(
    ...
    try {
      ...
      if (fieldsParam != null && fieldsParam.trim().isNotEmpty) {
        debugPrint('📌 Using GetIt service for field filtering: fields=$fieldsParam, id=$id');
      ...
      if (statusCode == 400) {
        troubleshootingTip = "Bad request. Check if the API version '${ApiNetwork.apiVersion}' is valid and the application charge ID exists.";
  ...
  Future<Map<String, dynamic>> _handleFieldFiltering(int id, String fields) async {
    ...
    try {
      ...
      debugPrint('✅ Successfully retrieved and filtered application charge. Fields: ${requestedFields.join(', ')}');
      
      return {
        "status": "success",
        "application_charge": filteredCharge,
        "fields_filtered": requestedFields.toList(),
        "message": "Application charge successfully retrieved with filtered fields",
        "timestamp": DateTime.now().toIso8601String(),
      };
    } catch (e) {
      ...
      return {
        "status": "error",
        "message": "Failed to retrieve application charge with field filtering: ${e.toString()}",
        "timestamp": DateTime.now().toIso8601String(),
      };
    }
  }
