...
class RetrieveARecurringApplicationChargeHandler implements ApiRequestHandler {
  ...
  Future<Map<String, dynamic>> _handleFieldFiltering(int id, String fields) async {
    ...
    try {
      ...
      debugPrint('✅ Successfully retrieved and filtered recurring application charge. Fields: ${requestedFields.join(', ')}');
      
      return {
        "status": "success",
        "recurring_application_charge": filteredCharge,
        "fields_filtered": requestedFields.toList(),
        "message": "Recurring application charge successfully retrieved with filtered fields",
        "timestamp": DateTime.now().toIso8601String(),
      };
    } catch (e) {
      ...
      return {
        "status": "error",
        "message": "Failed to retrieve recurring application charge with field filtering: ${e.toString()}",
        "timestamp": DateTime.now().toIso8601String(),
      };
    }
  }
