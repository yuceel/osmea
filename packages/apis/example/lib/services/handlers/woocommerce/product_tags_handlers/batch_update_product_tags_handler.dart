...
///*******************************************************************
//************* 📦 BATCH UPDATE PRODUCT TAGS HANDLER ***************
///*******************************************************************

class BatchUpdateProductTagsHandler implements ApiRequestHandler {
  @override
  Future<Map<String, dynamic>> handleRequest(
    ...
    if (method != 'PATCH') {
      return {
        "status": "error",
        "message": "Method $method not supported for Batch Update Product Tags API",
    ...
    try {
      ...
      final batchData = BatchUpdateProductTagsRequest(
        create: createOperations,
        update: updateOperations,
        delete: deleteOperations,
      );

      final response = await GetIt.I<ProductTagsService>().batchUpdateProductTags(
      ...
      return {
        "status": "error",
        "message": "Failed to batch update product tags: ${e.toString()}",
        "timestamp": DateTime.now().toIso8601String(),
      };
    }
  }
  ...
  Map<String, List<ApiField>> get requiredFields => {
    'PATCH': [
      const ApiField(
        ...
        hint: 'Create operations in format: name:slug:description,name2:slug2:description2',
        isRequired: false,
      ),
      const ApiField(
        name: 'update_operations',
        label: 'Update Operations',
        hint: 'Update operations in format: id:name:slug:description,id2:name2:slug2:description2',
        isRequired: false,
      ),
      const ApiField(
        name: 'delete_operations',
        label: 'Delete Operations',
        hint: 'Comma-separated list of tag IDs to delete: 1,2,3',
        isRequired: false,
      ),
      const ApiField(
        name: 'api_version',
