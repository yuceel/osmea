...
class BatchUpdateProductReviewsHandler implements ApiRequestHandler {
  ...
  Future<Map<String, dynamic>> handleRequest(
    ...
    if (method != 'PATCH') {
      return {
        "status": "error",
        "message": "Method $method not supported for Batch Update Product Reviews API",
    ...
    try {
      ...
      final response = await GetIt.I<ProductReviewsService>().batchUpdateProductReviews(
        ...
        .map((operation) {
          ...
          if (parts.length >= 2) {
            return Update(
              ...
              productId: parts.length > 1 ? int.tryParse(parts[1].trim()) : null,
  ...
  Map<String, List<ApiField>> get requiredFields => {
        'PATCH': [
          const ApiField(
            ...
            hint: 'Create operations in format: productid:status:reviewer:email:review:rating:verified,productid2:status2:reviewer2:email2:review2:rating2:verified2',
            isRequired: false,
          ),
          const ApiField(
            name: 'update_operations',
            label: 'Update Operations',
            hint: 'Update operations in format: id:productid:status:reviewer:email:review:rating:verified,id2:productid2:status2:reviewer2:email2:review2:rating2:verified2',
            isRequired: false,
          ),
          const ApiField(
