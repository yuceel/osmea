...
class BatchUpdateProductShippingClassesHandler implements ApiRequestHandler {
  ...
  Future<Map<String, dynamic>> handleRequest(
    ...
    if (method != 'PATCH') {
      return {
        "status": "error",
        "message": "Method $method not supported for Batch Update Product Shipping Classes API",
    ...
    try {
      ...
      final response = await GetIt.I<ProductShippingClassesService>().batchUpdateProductShippingClasses(
      ...
      return {
        "status": "error",
        "message": "Failed to batch update product shipping classes: ${e.toString()}",
        "timestamp": DateTime.now().toIso8601String(),
      };
    }
  }
