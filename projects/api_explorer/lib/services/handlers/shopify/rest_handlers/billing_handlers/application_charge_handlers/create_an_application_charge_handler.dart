...
import 'package:apis/network/remote/shopify/rest/billing/application_charge/freezed_model/request/create_application_charge_request.dart';
      ...
      if (method == 'POST') {
        ...
        if (returnUrl == null || returnUrl.isEmpty) {
          return {
            "status": "error",
            "message":
                "Return URL is required for creating an application charge",
            "timestamp": DateTime.now().toIso8601String(),
          };
        }
        ...
        bool? test = testStr == null
            ? true
            : testStr.toLowerCase() == 'true'; // Default to true for test
        String? currency =
            params['currency'] as String? ?? 'USD'; // Default to USD

        debugPrint(
            '🔧 Processed parameters - name: $name, price: $price, returnUrl: $returnUrl, test: $test, currency: $currency');
        ...
        if (charge == null) {
          return {
            "status": "error",
            "message":
                "Failed to create application charge - no charge data in response",
            "timestamp": DateTime.now().toIso8601String(),
          };
        }
        ...
        final confirmationUrl =
            charge.confirmationUrl ?? "No confirmation URL provided";
  ...
  Map<String, List<ApiField>> get requiredFields => {
        'POST': [
          const ApiField(
            ...
            label: 'Name',
            hint:
                'The name of the application charge (e.g., "Super Duper Expensive action")',
            isRequired: true,
          ),
          const ApiField(
            name: 'price',
          ...
          const ApiField(
            ...
            label: 'Return URL',
            hint:
                'The URL to redirect to after the charge is accepted/declined',
            isRequired: true,
          ),
          const ApiField(
            name: 'test',
            label: 'Test Mode',
            hint:
                'Set to true for a test charge that won\'t be billed (default: true)',
          ...
          const ApiField(
            ...
            label: 'Currency',
            hint:
                'The 3-letter currency code (e.g., USD, EUR, GBP) (default: USD)',
            isRequired: false,
          ),
        ],
      };
