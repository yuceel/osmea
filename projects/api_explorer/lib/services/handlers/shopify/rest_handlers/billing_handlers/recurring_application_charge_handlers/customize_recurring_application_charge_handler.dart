  ...
  Future<Map<String, dynamic>> handleRequest(
    ...
    try {
      if (method == 'PUT') {
        ...
        debugPrint(
            '🔧 Customizing recurring application charge ID $id with capped_amount=$cappedAmountStr');
        ...
        if (charge == null) {
          return {
            "status": "error",
            "message":
                "Failed to customize recurring application charge - no data in response",
            "timestamp": DateTime.now().toIso8601String(),
          };
        }
      ...
      debugPrint(
          '❌ Error customizing recurring application charge: ${e.toString()}');
      return {
        "status": "error",
        "message":
            "Failed to customize recurring application charge: ${e.toString()}",
        "apiVersion": ApiNetwork.apiVersion,
        "params": params,
        "timestamp": DateTime.now().toIso8601String(),
      };
